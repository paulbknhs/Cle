namespace Cle.Classes{
  ...
  public static class Statistics{
    ...
    public static DataTable GründeFürErziehung() {
      ...
      var grundSgb8Index
        = (from DataRow row in dataTable.Rows select row[0].ToString()[0].ToString()).ToDictionary(
          (string c) => c,
          (string c) => grundSgb8Dict[c]);
    ...
    public static DataTable SchwangerschaftAufteilung() {
      ...
      var anzahlSchwangerDict = new Dictionary<string, int> {
                                                              {
                                                                "Mutter-Kind-Antragsstellung",
                                                                db.GetNumberOfRows("Mutter Kind")
                                                              }, {
                                                                   "Allg. Schwangerschaftsberatung",
                                                                   db.GetNumberOfRows("Allgemeine Schwangerschaft")
                                                                 }, {
                                                                      "Schwangerschaftskonfliktberatung §218",
                                                                      db.GetNumberOfRows("§ 218")
                                                                    }
                                                            };
      db.Disconnect();
