namespace Cle.Classes
{
  ...
  public static class ReadInput
  {
    public static void AgeFromGridView(Control frame, Dictionary<string, string> dictionary)
    {
      foreach (Control control in frame.Controls)
      {
        ...
        foreach (DataGridViewRow row in grid.Rows)
        {
          ...
          var value = row.Cells[1].Value?.ToString() ?? row.Cells[2].Value?.ToString() ?? row.Cells[3].Value?.ToString();

          foreach (DataGridViewCell cell in row.Cells)
          {
            if (cell == null || cell.Value == null || cell.Value.ToString().IsNullOrEmpty() || cell.ColumnIndex == 0)
              continue; // check for null cell
    ...
    public static void FromNumberBox(Control frame, Dictionary<string, string> dictionary)
    {
      foreach (Control control in frame.Controls)
      {
        if (control is not NumericUpDown numBox || numBox.Text == string.Empty) continue;
    ...
    public static void FromTextBox(Control frame, Dictionary<string, string> dictionary)
    {
      foreach (Control control in frame.Controls)
      {
        if (control is not RJTextBox tb || tb.Texts == string.Empty) continue;
    ...
    public static DialogResult LetUserVerify(Dictionary<string, string> dictionary)
    {
      ...
      var panel = new Panel { Dock = DockStyle.Bottom, Padding = new Padding(15) };
