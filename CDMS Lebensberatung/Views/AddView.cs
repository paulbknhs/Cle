...
using RJTextBox = CustomControls.RJControls.RJTextBox;
...
public partial class AddView : UserControl
{
    ...
    private void OnButtonSave(object sender, EventArgs e)
    {
        ...
        DSTR.StoreToggleContent(panelToggle, DICT.Allgemein);
        DSTR.StoreAlterGridContent(this, DICT.Allgemein);

        if (cbARGE.Checked)
            DICT.Allgemein.Add("ARGE", "Ja");

        DICT.Allgemein.Add("Age", GetAgeFromRow(GetRelevantRow()));
        DICT.Allgemein.Add("Gender", GetGenderFromRow(GetRelevantRow()));

        if (DSTR.ValidateEntries(DICT.Allgemein) != DialogResult.OK) return;

        DatabaseHelper database = new(ConfigurationManager.AppSettings.Get("ConnectionString"));
        database.Connect();
        database.InsertStringDict("Allgemein", DICT.Allgemein);
        database.Disconnect();
    ...
    private DataRow GetRelevantRow()
    {
        ...
        {
            tableGridView.Columns.Add(column.Name);
        }
